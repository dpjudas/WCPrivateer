...
int ToolApp::Main(std::vector<std::string> args)
{
	WriteOutput(ColorEscape(96) + "Welcome to the WCPrivateer tool!" + ResetEscape() + NewLine());
	WriteOutput(NewLine());
...
void ToolApp::CreateCommandlets()
{
...
void ToolApp::OnCommandEntered(const std::string& line)
{
	try
	{
		std::string command;
...
std::string ToolApp::ToFixed(float time)
{
...
/*
void ToolApp::PrintLog(const LogMessageLine& line)
{
	if (PromptLineActive)
	{
...
std::string ToolApp::ResetEscape()
{
	return "\x1b[0m";
}

std::string ToolApp::ColorEscape(int color)
{
	return "\x1b[" + std::to_string(color) + "m";
}

std::string ToolApp::DeleteCharacterEscape(int count)
{
	return "\x1b[" + std::to_string(count) + "P";
}

std::string ToolApp::EraseCharacterEscape(int count)
{
	return "\x1b[" + std::to_string(count) + "X";
}

std::string ToolApp::CursorBackward(int count)
{
	return "\x1b[" + std::to_string(count) + "D";
}

std::string ToolApp::NewLine()
{
#ifdef WIN32
	return "\r\n";
#else
...
std::string ToolApp::InputNewLine()
{
#ifdef WIN32
	return "\r";
#else
...
void ToolApp::WriteOutput(const std::string& text)
{
	if (text.empty())
		return;
